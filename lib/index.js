...
if (process.env.TEST_MODE) {
    ...
    openapi_result_fcty.preOutput = preOutput;
...
function openapi_result_fcty(fittingDef, bagpipes) {
    ...
    return fittingDef.beautifyJson
        ...
        var err = errorToOutputIfAny(ctx, fittingDef);
...
// use special client settings
function preOutput(err, ctx) {
    if (!ctx._preOutput)
        return;

    try {
        ctx._preOutput(err, ctx);
    } catch (e) {
        debug("unable use preOutput", e.message);
        ctx.statusCode = 500;
        ctx.output = {
            message: "error invoking _preOutput handler",
            error: e.message,
            stack: e.stack
        }
    }
...
function beautifyJsonContents(ctx, fittingDef) {
    ...
    try {
        ...
        ctx.output = JSON.stringify({
            ...
            inspect: util.inspect(ctx.output, {depth: 10, breakLength: 1})
                .split("\n")
        }, null, 2);
    }
}
